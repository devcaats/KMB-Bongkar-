...
export default function LaporanBongkar({
    deliveryOrders = [],
    nettoMuatByDo = {},
    unloadReports = [],
}) {
    ...
    const selectedDo = useMemo(
        () => deliveryOrders.find((d) => String(d.id) === String(data.do_id)),
        [data.do_id, deliveryOrders],
    );

    // Nomor unit auto-fill
    ...
    // Netto bongkar: bruto - tara
    const nettoBongkar = useMemo(() => {
        const b = parseFloat(data.bruto) || 0;
        const t = parseFloat(data.tara) || 0;
        return b - t > 0 ? b - t : 0;
    }, [data.bruto, data.tara]);

    // Netto muat from props (keyed by DO id)
    const nettoMuat = useMemo(() => {
        const doId = editingReport
            ? editingReport.delivery_order_id
            : data.do_id;
        if (!doId) return null;
        const val = nettoMuatByDo[String(doId)];
        return val != null ? parseFloat(val) : null;
    }, [data.do_id, editingReport, nettoMuatByDo]);

    // Selisih: netto bongkar - netto muat
    const selisih = useMemo(() => {
        if (nettoMuat == null || nettoBongkar === 0) return null;
        return nettoBongkar - nettoMuat;
    }, [nettoBongkar, nettoMuat]);

    const selisihStatus = useMemo(() => {
        if (selisih === null) return null;
        if (selisih > 0) return "surplus";
        if (selisih < 0) return "susut";
        return "pas";
    }, [selisih]);
    ...
    const startEdit = (report) => {
        ...
        setData({
            ...
            unload_date: report.unload_date,
    ...
    const handleDelete = () => {
        ...
        router.post(route("driver.bongkar.destroy.post", deleteTarget.id), {
            ...
            onSuccess: () => {
                setPopupState({
                    type: "success",
                    message: "Laporan bongkar berhasil dihapus.",
                });
            ...
            onError: () => {
                setPopupState({
                    type: "error",
                    message: "Gagal menghapus laporan bongkar.",
                });
                setDeleteTarget(null);
            },
        });
    ...
    const handleSubmit = (e) => {
        ...
        const url = editingReport
            ? route("driver.bongkar.update", editingReport.id)
            : route("driver.bongkar.store");
        ...
        post(url, {
            ...
            onSuccess: () => {
                const msg = editingReport
                    ? "Laporan bongkar berhasil diperbarui!"
                    : "Laporan bongkar berhasil dikirim ke sistem.";
    ...
    const formatKg = (val) =>
        typeof val === "number" || val != null
            ? parseFloat(val).toLocaleString("id-ID") + " kg"
            : "—";
    ...
    const selisihColor = (status) => {
        if (status === "surplus")
            return "text-success-600 dark:text-success-400";
        if (status === "susut") return "text-error-600 dark:text-error-400";
        return "text-gray-600 dark:text-gray-400";
    };

    const selisihBg = (status) => {
        if (status === "surplus")
            return "bg-success-50 dark:bg-success-500/10 border-success-200 dark:border-success-800";
        if (status === "susut")
            return "bg-error-50 dark:bg-error-500/10 border-error-200 dark:border-error-800";
        return "bg-gray-50 dark:bg-white/[0.02] border-gray-200 dark:border-gray-800";
    };

    // Check if form has valid selisih to display
    const showSelisih =
        selisih !== null &&
        (parseFloat(data.bruto) > 0 || parseFloat(data.tara) > 0);

    return (
        <AppLayout>
            <Head title="Laporan Bongkar" />
            ...
            >
                {popupState && (
                    <div
                        className={`relative w-full max-w-md mx-auto rounded-2xl shadow-2xl overflow-hidden bg-white dark:bg-gray-900 border ${
                ...
                {viewingPhoto && (
                    <div className="relative max-w-2xl mx-auto bg-white dark:bg-gray-900 rounded-2xl overflow-hidden p-3 shadow-2xl">
                        <button
                            ...
                            className="absolute top-4 right-4 z-10 h-8 w-8 bg-black/60 hover:bg-black/80 text-white rounded-full flex items-center justify-center transition-colors"
                        >
                            <svg
                                className="h-5 w-5"
                                fill="none"
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-extrabold text-gray-800 dark:text-white">
                            {editingReport
                                ? "Edit Laporan Bongkar"
                                : "Laporan Bongkar Barang"}
                        </h1>
                        <p className="text-xs text-gray-400 mt-0.5">
                            {editingReport
                                ? `Mengubah data bongkar untuk DO #${editingReport.delivery_order?.nomor_do}`
                                : "Kirim data timbangan bongkar di lokasi tujuan pengiriman"}
                        </p>
                    </div>
                </div>

                {/* ── Empty State (no DOs with load reports) ── */}
                {deliveryOrders.length === 0 && !editingReport ? (
                    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-10 text-center space-y-4 shadow-sm">
                        <div className="mx-auto w-16 h-16 rounded-full bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-500">
                            <svg
                                className="h-8 w-8"
                                fill="none"
                                viewBox="0 0 24 24"
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-gray-850 dark:text-white/90">
                                Belum Ada DO Siap Dibongkar
                            </h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-sm mx-auto">
                                Tidak ada Delivery Order yang sudah memiliki
                                laporan muat aktif. Pastikan Anda sudah mengisi{" "}
                                <strong>Laporan Muat</strong> terlebih dahulu
                                sebelum membuat laporan bongkar.
                            </p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden shadow-sm">
                        <div className="px-6 py-4 border-b border-gray-150 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.02] flex justify-between items-center">
                            <h3 className="text-sm font-bold text-gray-800 dark:text-white/90">
                                {editingReport
                                    ? "Formulir Edit Laporan Bongkar"
                                    : "Formulir Laporan Bongkar"}
                            </h3>
                            {editingReport && (
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setData("unload_date", e.target.value)
                                    }
                            ...
                            {/* Bruto & Tara Bongkar (1 row) */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {/* Bruto */}
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-semibold text-gray-755 dark:text-gray-300">
                                        Timbangan Bruto Bongkar (Kg){" "}
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        value={
                                            nettoBongkar > 0
                                                ? `${nettoBongkar.toLocaleString("id-ID")} kg`
                                                : "0 kg"
                                        }
                                        className="h-11 w-full rounded-xl border border-gray-200 bg-gray-50 dark:bg-gray-900/50 px-4 text-sm font-bold text-indigo-600 dark:text-indigo-400 focus:outline-none dark:border-gray-700"
                                    />
                            ...
                            {/* Plus/Minus Bongkar (Selisih) */}
                            {showSelisih && (
                                <div
                                    className={`flex items-center gap-4 rounded-xl border px-5 py-4 ${selisihBg(selisihStatus)}`}
                                >
                                    <div
                                        className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-extrabold text-lg ${
                                            selisihStatus === "surplus"
                                    ...
                                    <div>
                                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                            Plus/Minus Bongkar (Selisih Netto)
                                        </p>
                                        <p
                                            className={`text-xl font-extrabold mt-0.5 ${selisihColor(selisihStatus)}`}
                                        >
                                            {selisih !== null
                                                ? (selisih >= 0 ? "+" : "") +
                                        ...
                                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                                            {selisihStatus === "surplus"
                                                ? "Surplus — Netto bongkar melebihi netto muat"
                                                : selisihStatus === "susut"
                                                  ? "Susut — Netto bongkar lebih kecil dari netto muat"
                                                  : "Pas — Netto bongkar sesuai dengan netto muat"}
                                        </p>
                                        {nettoMuat != null && (
                                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                                                Netto Muat:{" "}
                                                <span className="font-semibold">
                                                    {nettoMuat.toLocaleString(
                                                        "id-ID",
                                                    )}{" "}
                                                    kg
                                                </span>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <textarea
                                    ...
                                    placeholder="Tambahkan catatan jika ada kendala saat pembongkaran atau selisih berat..."
                                    rows={3}
                            ...
                            <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3">
                                {editingReport ? (
                                    ...
                                    <>
                                        <Link
                                            href="/dashboard"
                                            className="px-4 py-1.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05] transition-colors"
                                        >
                                            Batal
                                        </Link>
                                        <button
                                            type="submit"
                                            disabled={processing}
                                            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-xl bg-indigo-600 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md"
                                        >
                                            {processing ? (
                                                <svg
                                                    className="h-4 w-4 animate-spin"
                ...
                {/* ── Riwayat Laporan Bongkar ── */}
                {unloadReports.length > 0 && (
                    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden shadow-sm">
                        <div className="px-6 py-4 border-b border-gray-150 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.02] flex justify-between items-center">
                            <div>
                                <h3 className="text-sm font-bold text-gray-800 dark:text-white/90">
                                    Riwayat Laporan Bongkar Anda
                                </h3>
                                <p className="text-[11px] text-gray-400 mt-0.5">
                                    Daftar laporan pembongkaran yang pernah Anda
                                    kirimkan
                                </p>
                            </div>
                            <span className="px-2.5 py-1 text-xs font-semibold bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 rounded-lg">
                                Total: {unloadReports.length} Laporan
                            </span>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                    {unloadReports.map((report, index) => (
                                        <tr
                                            key={report.id}
                                            className={`hover:bg-gray-50/30 dark:hover:bg-white/[0.005] transition-colors text-sm ${
                                            ...
                                            <td className="px-6 py-4 space-y-0.5">
                                                <div className="text-xs text-gray-500 dark:text-gray-400">
                                                    ...
                                                    <span className="font-medium text-gray-700 dark:text-gray-300">
                                                        {formatKg(report.bruto)}
                                                    </span>
                                                </div>
                                                <div className="text-xs text-gray-500 dark:text-gray-400">
                                                    Tara:{" "}
                                                    <span className="font-medium text-gray-700 dark:text-gray-300">
                                                        {formatKg(report.tara)}
                                                    </span>
                                                </div>
                                                <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                                                    Netto:{" "}
                                                    {formatKg(report.netto)}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <span
                                                    className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold border ${
                                                        report.status_selisih ===
                                                        "surplus"
                                                            ? "bg-success-50 text-success-700 border-success-200 dark:bg-success-500/10 dark:text-success-400 dark:border-success-800"
                                                            : report.status_selisih ===
                                                                "susut"
                                                              ? "bg-error-50 text-error-700 border-error-200 dark:bg-error-500/10 dark:text-error-400 dark:border-error-800"
                                                ...
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        ...
                                                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                                                            editingReport?.id ===
                                                            report.id
                                                                ? "bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 cursor-default"
                                                    ...
                                                    <button
                                                        ...
                                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-error-50 hover:bg-error-100 text-error-700 dark:bg-error-500/10 dark:hover:bg-error-500/20 dark:text-error-400 transition-colors"
                                                    >
                                                        <svg
                                                            className="h-3.5 w-3.5"
                                                            fill="none"
